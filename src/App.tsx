    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F9FBFF] p-6 animate-in">
      <div className="max-w-md w-full bg-white rounded-[3.5rem] shadow-2xl p-10 sm:p-14 border border-slate-100">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          ...
          <input required type="email" placeholder="User Email" className="w-full bg-slate-50 border-2 border-transparent focus:border-[#1E3A8A] rounded-[1.5rem] px-6 py-4 text-xs font-bold outline-none transition-all" value={email} onChange={(e) => setEmail(e.target.value)} />
        ...
        <div className="mt-8 text-center">
          <button onClick={() => setIsLogin(!isLogin)} className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-[#1E3A8A] transition-colors">{isLogin ? "New User? Sign Up" : "Registered User? Login"}</button>
...
const App: React.FC = () => {
  ...
  const executeCalculation = async () => {
    if (isGuest && guestCount >= GUEST_LIMIT) {
      alert(`Free limit of ${GUEST_LIMIT} estimates reached. Please Sign Up as a Premium User to continue.`);
      return;
    }
